...
Usage:
    ...
    python detector.py train --model=coco --dataset=../data --class_map=./taco_config/map_10.csv --round 0
    # Continue training a model that you had trained earlier
    python3 -W ignore detector.py train  --dataset=../data --model=<model_name> --class_map=./taco_config/map_10.csv --round 0
    python -W ignore detector.py train  --dataset=../data --model=taco20210529T1930 --class_map=./taco_config/map_10.csv --round 0
    ...
    python detector.py test --dataset=../data --model=mask_rcnn_taco_0100 --round 0 --class_map=./taco_config/map_10.csv
    ...
    # Check Tensorboard
    tensorboard --logdir ./models/logs/taco20210529T1945 --host localhost --port 8088
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        required=True,
        metavar="/path/weights.h5",
        help="Path to weights .h5 file or 'coco'",
    )
    parser.add_argument(
        "--dataset", required=True, metavar="/path/dir", help="Directory of the dataset"
    )
    parser.add_argument("--round", required=True, type=int, help="Split number")
    parser.add_argument(
        "--lrate", required=False, default=0.001, type=float, help="learning rate"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to transplanted dataset",
    )
    parser.add_argument(
        "--class_map", required=True, metavar="/path/file.csv", help=" Target classes"
    )
    ...
    if args.command == "train":
        ...
        dataset_train.load_taco(
            args.dataset, args.round, "train", class_map=class_map, auto_download=None
        )
        if args.use_transplants:
            dataset_train.add_transplanted_dataset(
                args.use_transplants, class_map=class_map
            )
